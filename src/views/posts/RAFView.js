...
export default function RAFView() {
	return (
		<PageCentered>
			...
			<Paragraph>If you have been anywhere but under a rock then you have heard about Javascript’s new requestAnimationFrame function. You have probably used the <Link href="http://paulirish.com/2011/requestanimationframe-for-smart-animating/">shim posted by Paul Irish</Link>, may have tinkered with the <Link href="http://my.opera.com/emoller/blog/2011/12/20/requestanimationframe-for-smart-er-animating">version created by Opera developer Erik Möller</Link>, or even read <Link href="http://www.html5rocks.com/en/tutorials/speed/animations/">various</Link> articles <Link href="http://updates.html5rocks.com/2012/05/requestAnimationFrame-API-now-with-sub-millisecond-precision">explaining</Link> how to <Link href="http://creativejs.com/resources/requestanimationframe/">best use it</Link>.</Paragraph>
			...
			<Paragraph>I’m just as guilty of abusing requestAnimationFrame as anyone else. In my <Link href="http://chandlerprall.github.com/Physijs/">physics plugin</Link> for Three.js I’m still using requestAnimationFrame to schedule both the physics updates as well as the rendering. The physics is handled in a web worker to avoid slowing the graphics framerate, but that simulation is still bound to the rendering because it is called from requestAnimationFrame. If the physics engine isn’t ready for another update until after the graphics render then it will sit around doing nothing for 10-15ms until requestAnimationFrame fires again. Changing the simulation loop to use setTimeout separates the logic from the graphics code and speeds up the whole demo by more than 30% in my tests – increasing the simulation’s effective FPS from 14 to over 20.</Paragraph>

			<Paragraph>One common way to use requestAnimationFrame for game logic is to use a delta value to determine how much time has actually passed since the last update; in fact this is a smart thing to do whether your use the requestAnimationFrame or setTimeout method. However, relying on just this delta means your logic code is still bound to the graphics loop which can cause unexpected results – say in a multiplayer environment when the user will want their actions to be reflected immediately on the server instead of waiting for some graphics layer to first run.</Paragraph>

			<Paragraph>You might think that this use of requestAnimationFrame is only an isolated issue but recently more people have been noticing its impact. Just a couple of days ago someone came into the #threejs IRC channel with this question:</Paragraph>

			<quote>
				Hi there, I’ve a short question…

				I recently updated my code using requestAnimationFrame(), now my animation runs as fast as possible, where before it was running at fixed 30fps

				i implemented a navigation through the space using the cursor-keys, but now the navigation-speed depends on the speed of the browser/computer
			</quote>
